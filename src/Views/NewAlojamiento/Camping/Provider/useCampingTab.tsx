...
const campingSchema = z.object({
  ...
  cantidad_cama_doble: numeric,
  cantidad_cama_individual: numeric,
  cantidad_sofa_cama: numeric,

  cantidad_baños: numeric,
  baño_adaptado: z.boolean(),
...
export type CampingContextValue = {
  datosRegistradosAlojamiento:
    | TObtenerDatosRegistradosAlojamientoResponse
    | undefined;
  datosRegistroAlojamiento: any;
  guardarAlojamiento: (body: TBodyGuardarAlojamiento) => Promise<void>;
  campingSchema: typeof campingSchema;
  alojamientoForm: UseFormReturn<z.infer<typeof campingSchema>>;
...
const useCampingTab = ({ idOferta }: { idOferta: string }) => {
  ...
  const handleObtenerDatosRegistradosAlojamiento = () => {
    obtenerDatosRegistradosAlojamiento(idOferta).then(
      (response: TAdaptedObtenerDatosRegistradosAlojamientoResponse) => {
        ...
        setEsCompleto(campingSchema.safeParse(reset).success);
      }
    );
  };
  ...
  const alojamientoForm = useForm<z.infer<typeof campingSchema>>({
  ...
  React.useEffect(() => {
    setEsCompleto(
      campingSchema.safeParse(alojamientoForm.getValues()).success &&
        !alojamientoForm.formState.isDirty
    );
  }, [alojamientoForm.formState]);

  const context: CampingContextValue = {
    campingSchema,
    alojamientoForm,
