...
export const useProvideAuth = (props: any) => {
		...
		getDatosUsuario()
			.then((response: any) => {
				...
				if (response.data.tiene_perfil) {
					if (
						response.data.perfilesUsuario[0].id_perfil ==
						PERFILES.TURISTA.id
					) {
						usuario = {
							nombre: response.data.datos_usuario_perfil
								.datos_turista.nombre,
							apellido:
								response.data.datos_usuario_perfil.datos_turista
									.apellido,
							mail: response.data.datos_usuario_perfil
								.datos_turista.mail,
						};
					} else {
						usuario = {
							nombre: response.data.datos_usuario_perfil
								.datos_prestador.nombre,
							apellido:
								response.data.datos_usuario_perfil
									.datos_prestador.apellido,
							mail: response.data.datos_usuario_perfil
								.datos_prestador.mail,
						};
					}
				}
				...
				const perfilSeleccionado =
					perfiles.length > 0 ? perfiles[0].id_perfil : -1;
	...
	const getPerfil = (): number => {
		...
		return data.perfiles[0].id_perfil;
	};
